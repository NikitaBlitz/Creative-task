...
namespace Кредитныйкалькулятор {
	using namespace std;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label2->Size = System::Drawing::Size(348, 28);
			this->label2->TabIndex = 4;
			this->label2->Text = L"Годовая ставка по кредиту";
			...
			this->label3->Size = System::Drawing::Size(339, 28);
			this->label3->TabIndex = 6;
			this->label3->Text = L"Срок выплаты (в месяцах)";
			// 
			// label4
			// 
			...
			this->label4->Size = System::Drawing::Size(276, 28);
			this->label4->TabIndex = 7;
			this->label4->Text = L"Ежемесячный платеж";
			// 
			// textBox4
			// 
			...
			this->label5->Text = L"Общая выплата";
			// 
			// label6
			// 
			...
			this->label6->Text = L"Переплата по кредиту";
			// 
			// textBox6
			// 
			...
			this->ClientSize = System::Drawing::Size(1050, 383);
...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e)
{
	if (Check())
	{
		...
		double sum, stavka, result; // сумма кредита, годовая ставка, результат
		int month; //срок в месяцах 
		...
		textBox5->Text = Convert::ToString(round(result * month*100)/100);
		textBox6->Text = Convert::ToString(round((result * month - sum) * 100)/100);
	}
	else
	{
			MessageBox::Show("Неправильно введенны данные", "Ошибка!!!");
	}
}
double math(double sum, double stavka, int month) // функция нахождения ежемесячного платежа
{
	return sum * ((stavka / (12 * 100)) + (stavka / (12 * 100)) / (pow(1 + (stavka / (12 * 100)), month) - 1));
}
bool Check() // функция проверки
{
		   int k = 0; //счетчик запятых, нужно либо 0 или 1 запятая
