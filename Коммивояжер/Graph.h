...
int n;//Кол-во вершин
int** tmp;//Дополнительный массив для оценок пути
...
{
	int R_;
	int x0 = 1000 / 2;//Расположение центра
	int y0 = 800 / 2;
...
void Circle_Edge_Text(int nom, int x1, int y1)//Функция отрисовки текста в вершинах и на рёбрах
{
...
void drawVertex(int n)//Функция отрисовки вершин с текстом
{
	for (int i = 0; i < n; i++)//Цикл от кол-ва вершин
	{
		...
		Circle_Edge_Text(i + 1, vertC[i].x, vertC[i].y);//Функция отрисовки текста в вершинах и на рёбрах
	}
}
...
void drawLine(int text, int x0, int y0, int x1, int y1)//Функция отрисовки линий, соединяющих две вершины
{
	...
	glEnd();
	Circle_Edge_Text(text, (x0 + x1) / 2 + 12, (y0 + y1) / 2 + 12);//Отрисовка текста для ребра
}
...
void Text(string text, int x1, int y1)//Функция отрисовки текста в меню
{
	string s = text;
...
void Menu()//Отрисовка меню с кнопками
{
	...
	glColor3d(0, 0, 0);
	Text("New matrix", 0, 800); 
	glColor3d(0, 0, 0);
	Text("Print matrix", 0, 770);
	glColor3d(0, 0, 0);
	Text("Answer", 0, 740);
} 
